...
Currently tailstream supports the following output types:
...
- `http[s]://[<username>][:<password>@]<hostname>[:port][?tenant_id=<tenant>]`. Pushes logs to Grafana Loki via the `/loki/api/v1/push` endpoint. Supports basic authentication and multi-tenancy via the `X-Scope-OrgID` header.

> **Note**: Loki output ignores the compression flag as it sends JSON payloads with structured log data.
...
OPTIONS:
    ...
    -o, --output <OUTPUT>              Output stream. defaults to console://. Other output supports
                                       ...
                                       http[s]://[user:password@]<address[:port]>[?tenant_id=<tenant>]
                                       [default: console://]
